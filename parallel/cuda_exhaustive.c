...
void propagation(double *masses, double *positions, double *velocities, uint ents_sz, size_t dt);
void safe_malloc_double(double **pointer, int quantity);
void update_positions(double *positions, double *velocities, uint ents_sz, size_t dt);
...
void run(double *masses, double *positions, double *velocities, uint ents_sz, size_t t_start, size_t t_end, size_t dt, const char *output) {
    ...
    for(size_t t = t_start; t<t_end; t+=dt) { //Ciclo sul tempo
        propagation(masses, positions, velocities, ents_sz, dt);
        update_positions(positions, velocities, ents_sz, dt);
        write_positions(positions, velocities, ents_sz, fpt);
    }
    fclose(fpt);
}
