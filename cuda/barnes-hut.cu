	...
	while (!allocated) {
		...
		if (child_val != LOCKED) {
			...
			if (child_val ==
				...
				} else if (node->children[child_indx] < tree->root) {
					allocated = 0;
...
__global__ void get_bounding_box(double *ents, int ents_sz, uint padding) {
	...
	if (t < sz) {
		...
		// initialize value to 0 (the smallest possible value) to avoid problem
		// with reduce
		partial_max[t] = 0;
	}
	...
	for (uint stride = blockDim.x / 2; stride >= 1; stride >>= 1) {
		__syncthreads();
		if (t < stride) {
			partial_max[t] = partial_max[t] > partial_max[t + stride]
								 ? partial_max[t]
								 : partial_max[t + stride];
		}
	}
	if (t == 0) {
...
__global__ void set_branch_values(Octtree *tree, int sz_threads) {
    //used as cache
...
__device__ void calculate_acceleration(double pos_node[], double mass_node, double pos_ent[], double mass_ent
	...
	r_vector[0] = tree->center[ent * 3] - tree->center[node * 3];
	r_vector[1] = tree->center[ent * 3 + 1] - tree->center[node * 3 + 1];
	r_vector[2] = tree->center[ent * 3 + 2] - tree->center[node * 3 + 2];

	double r_mag = sqrt(r_vector[0] * r_vector[0] + r_vector[1] * r_vector[1] +
						r_vector[2] * r_vector[2]);

	double acceleration = -1.0 * BIG_G * (tree->mass[node]) / (r_mag * r_mag);
...
// idea: a block for each body and use the thread to calculate acceleration
// idea: wait for all thread to find something on which calculate acceleration
__global__ void get_acceleration(Octtree *tree, Entities *ents, int ents_sz,uint threads_sz) {
    ...
    double pos_ent[3], vel_ent[3], mass_ent, border, acc_ent[3];
    ...
    for(int i=tid; i<ents_sz; i+=thread_sz){
        ...
        while(depth>=0){
            for(int j=0; j<8; j++){
                if(id_in_warp==0){
                    ...
                    pos_node[warp_id*3]=tree->pos[cache_node[warp_id]*3];
                    pos_node[warp_id*3+1]=tree->pos[cache_node[warp_id]*3+1];
                    pos_node[warp_id*3+2]=tree->pos[cache_node[warp_id]*3+2];
                    mass_node[warp_id]=tree->mass[cache_node];
                }
                __threadfence_block();
                if(cache_node[warp_id]>=0){
                    double distance=get_distance(pos_ent, pos_node);
                    if((border/depth)/<THETA){
